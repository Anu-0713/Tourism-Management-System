    ...
    <td width="66%" height="320" valign="top"> 
      <table align="center" bgcolor="#ffffcc" border="0" bordercolor="#ffffff" cellpadding="3" cellspacing="10" height="320" width="677">
        ...
        <tr> 
          ...
          <fieldset>
          <legend><strong>ADD TRANSPORT</strong></legend>
            
            <%@ page import="com.dts.project.dao.*"%>
<%@ page import="com.dts.project.model.*"%>
<%
            if(request.getParameter("AddTransport")!=null)
            {
            String sno=request.getParameter("tid");
             String city=request.getParameter("city");
             String carname=request.getParameter("carname");
             String carmodel=request.getParameter("carmodel");
             String cost1day=request.getParameter("cost1day");
             String costhalfday=request.getParameter("costhalfday");
             String cost1hr=request.getParameter("cost1hr");
             String cost1km=request.getParameter("cost1km");
               TransportModel atransportmodel=new TransportModel();
               atransportmodel.setTourorganisationid(sno);
               atransportmodel.setCity(city);
            atransportmodel.setCarname(carname);
            
            atransportmodel.setCarmodel(carmodel);
            atransportmodel.setCost1day(Double.parseDouble(cost1day));
            atransportmodel.setCosthalfday(Double.parseDouble(costhalfday));
            atransportmodel.setCost1hr(Double.parseDouble(cost1hr));
            atransportmodel.setCost1km(Double.parseDouble(cost1km));
            
            TransportDAO atransportdao=new TransportDAO();
           boolean flag= atransportdao.addTransport(atransportmodel);
             if(flag)
             
             {
             out.println("<font color=blue>RECORD IS INSERTING SUCCESSFUL</FONT>");
             }
            else
            {
             out.println("<font color=blue>RECORD IS INSERTING IS FAILED</FONT>");
             }}
           %> 
            
            <%
            if(request.getParameter("UpdateTransport")!=null)
            {
            String sno=request.getParameter("tid");
             String city=request.getParameter("city");
             String carname=request.getParameter("carname");
            ...
            TransportDAO atransportdao=new TransportDAO();
           boolean flag= atransportdao.updateTransport(atransportmodel);
             if(flag)
             
             {
             out.println("<font color=blue>RECORD IS UPDATAING SUCCESSFUL</FONT>");
             }
            else
            {
             out.println("<font color=blue>RECORD IS UPDATING IS FAILED</FONT>");
             }}
           %> 
            ...
            {
            String tod=request.getParameter("tid"); 
            
            TransportDAO atransportdao=new TransportDAO();
           boolean flag= atransportdao.deleteTransport(tod);
             if(flag)
             
             {
             out.println("<font color=blue>RECORD IS DELETED SUCCESSFUL</FONT>");
             }
            else
            {
             out.println("<font color=blue>RECORD IS DELETED IS FAILED</FONT>");
             }}
           %> 
