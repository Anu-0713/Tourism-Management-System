    ...
    <td width="66%" height="320" valign="top"> 
      <table align="center" bgcolor="#ffffcc" border="0" bordercolor="#ffffff" cellpadding="3" cellspacing="10" height="320" width="677">
        ...
        <tr> 
          ...
          <fieldset>
          <legend><strong>Sample Form</strong></legend>
         
         <%@ page import="com.dts.project.dao.*"%>
<%@ page import="com.dts.project.model.*"%>   <%if(request.getParameter("ShopInsert")!=null)
            {
         ...
         ashopping.setSno(sno);
         ashopping.setCity(city);
         ashopping.setShopname(shopname);
         ashopping.setArea(area);
         ashopping.setFamous(famous);
         ashopping.setDiscount(discount);
         ShoppingPlaces ashoppingplaces=new ShoppingPlaces();
         boolean flag=ashoppingplaces.addShopping(ashopping);
        if(flag)
          ...
          <%if(request.getParameter("ShopUpdate")!=null)
            {
            String sno=request.getParameter("sno");
             String city=request.getParameter("city");
             String shopname=request.getParameter("shopname");
         ...
         ashopping.setDiscount(discount);
         ShoppingPlaces ashoppingplaces=new ShoppingPlaces();
         boolean flag=ashoppingplaces.updateShopping(ashopping);
        if(flag)
             
             {
             out.println("<font color=blue>RECORD IS UPDATION SUCCESSFUL</FONT>");
             }
            else
            {
             out.println("<font color=blue>RECORD IS UPDATION IS FAILED</FONT>");
             }
             }
              %>
         
         <%if(request.getParameter("Shopdelete")!=null)
        ...
        Shopping ashopping=new Shopping();
         ashopping.setSno(sno);
         boolean flag=ashoppingplaces.deleteShopping(ashopping);
        if(flag)
