    ...
    <td width="66%" height="320" valign="top"> 
      <table align="center" bgcolor="#ffffcc" border="0" bordercolor="#ffffff" cellpadding="3" cellspacing="10" height="320" width="677">
        ...
        <tr> 
          ...
          <fieldset>
          <legend><strong>THEATERS ADMIN</strong></legend>
            <%@ page import="com.dts.project.model.*" %>
            <%@ page import="com.dts.project.dao.*" %>
            <%
            if(request.getParameter("theatersadd")!=null)
            {
            String sno=request.getParameter("sno");
             String city=request.getParameter("city");
             String theatername=request.getParameter("theatername");
             String cinema=request.getParameter("cinema");
             String morningshow=request.getParameter("morningshow");
             String matnyshow=request.getParameter("matnyshow");
             String firstshow=request.getParameter("firstshow");
             String secondshow=request.getParameter("secondshow");
             Enterainement atheaters=new Enterainement();
             atheaters.setSno(sno);
             atheaters.setCity(city);
             atheaters.setTheatername(theatername);
             atheaters.setCenima(cinema);
             atheaters.setMorningshow(morningshow);
             atheaters.setMatnyshow(matnyshow);
             atheaters.setFirstshow(firstshow);
             atheaters.setSecondshow(secondshow);
             Theaters atheaters1=new Theaters();
             boolean flag=atheaters1.addTheaters(atheaters);
             if(flag)
            ...
            <%
            if(request.getParameter("theatersupdate")!=null)
            {
            String sno=request.getParameter("sno");
             String city=request.getParameter("city");
             String theatername=request.getParameter("theatername");
             ...
             atheaters.setSecondshow(secondshow);
             Theaters atheaters1=new Theaters();
             boolean flag=atheaters1.updateTheaters(atheaters);
             if(flag)
             
             {
             out.println("<font color=blue>RECORD IS UPDATING SUCCESSFUL</FONT>");
             }
            else
            {
             ...
             <%if(request.getParameter("theatersdelete")!=null)
             {
             String sno=request.getParameter("sno");
            Theaters atheaters1=new Theaters();
             boolean flag=atheaters1.deleteTheaters(sno);
             if(flag)
             
             {
             out.println("<font color=blue>RECORD IS DELETION SUCCESSFUL</FONT>");
             }
            else
            {
             out.println("<font color=blue>RECORD IS DELETION IS FAILED</FONT>");
             }
             }
              %>
