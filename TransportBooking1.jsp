    ...
    <td width="66%" height="320" valign="top"> 
      <table align="center" bgcolor="#ffffcc" border="0" bordercolor="#ffffff" cellpadding="3" cellspacing="10" height="320" width="677">
        ...
        <tr> 
          ...
          <fieldset>
          <legend><strong>Selected Car</strong></legend>
            
            <%@  page import="com.dts.project.model.*" %>
 <%@ page import="com.dts.project.dao.*" %>
 <%@ page import="com.dts.core.util.*" %>
 <% 
  String tourrorganisationid=request.getParameter("tourid");
  ...
  String Car=request.getParameter("Carname");
  String country=request.getParameter("country");
  String cost1day=request.getParameter("cost1day");
  String cost1hr=request.getParameter("cost1hr");
  String carmodel=request.getParameter("carmodel");
  double totalcost=(Double.parseDouble(cost1day)*Double.parseDouble(days))+(Double.parseDouble(hours)*Double.parseDouble(cost1hr));
  	TransportModel atransportmodel=new TransportModel();
  	atransportmodel.setTourorganisationid(tourrorganisationid);
  	atransportmodel.setFirstname(firstname);
  	atransportmodel.setLastname(lastname);
  	atransportmodel.setCity(city);
  	atransportmodel.setState(state);
    atransportmodel.setCarname(Car);
    atransportmodel.setDays(Double.parseDouble(days));
        atransportmodel.setHours(Double.parseDouble(hours));
        atransportmodel.setTotalcost(totalcost);
        TransportDAO atransportdao=new TransportDAO();
        boolean flag=atransportdao.addTrnasportBooking(atransportmodel);
if(flag)
{
out.println("<table><tr><td>CarName</td><td>CarModel</td><td>Days</td><td>TotalCost</td></tr><tr><td>");
out.println(Car);
out.println("</td><td>");
